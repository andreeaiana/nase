# Adapted from https://github.com/UKPLab/sentence-transformers/blob/master/sentence_transformers/losses/DenoisingAutoEncoderLoss.py
...
class DenoisingAutoEncoderLoss(nn.Module):
    def __init__(
            self, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, decoder_name_or_path: Optional[str] = None, tie_encoder_decoder: bool = True
    ) -> None:
        """
        ...
        Args:
            model (SentenceTransformer): The Transformers model.
        ...
        encoder_name_or_path = model.base_model.config._name_or_path
        ...
        try:
            ...
            raise e
        assert model.base_model.config.hidden_size == decoder_config.hidden_size, "Hidden sizes do not match!"
    ...
    def forward(self, sentence_features: Iterable[Dict[str, Tensor]]):
        ...
        reps = self.encoder(**source_features).pooler_output  # (bsz, hdim)
